function generate_load_regressor(path_to_urdf, n_links)
% ----------------------------------------------------------------------
...
rpy_ee = sym(str2num(ur10.robot.joint{n_links+1}.origin.Attributes.rpy));
R_6ee = RPY(rpy_ee);
R_6ee(abs(R_6ee)<sqrt(eps)) = sym(0); % to avoid numerical errors
p_6ee = str2num(ur10.robot.joint{n_links+1}.origin.Attributes.xyz)';
T_6ee = sym([R_6ee, p_6ee; zeros(1,3), 1]); % to avoid numerical errors

w_eeee = T_6ee(1:3,1:3)'*w_kk(:,n_links+1);
v_eeee = T_6ee(1:3,1:3)'*(v_kk(:,n_links+1) + cross(w_kk(:,i+1),sym(p_6ee)));
g_eeee = T_6ee(1:3,1:3)'*g_kk(:,n_links+1);
p_eeee = T_6ee(1:3,1:3)'*(p_kk(:,n_links+1) + sym(p_6ee));
