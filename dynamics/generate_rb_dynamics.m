...
q_sym = sym('q%d',[dof,1],'real');
qd_sym = sym('qd%d',[dof,1],'real');
q2d_sym = sym('q2d%d',[dof,1],'real');

% ------------------------------------------------------------------------
% Getting gradient of energy functions, to derive dynamics
% ------------------------------------------------------------------------
T_pk = sym(zeros(4,4,dof)); % transformation between links
...
beta_Lf = [];
for i = 1:dof
    beta_Lf = [beta_Lf, beta_K(i,:) - beta_P(i,:)];
end
...
% Lagrangian dynamics
pi_sndrd_sym = sym('pi%d%d', [10*dof,1], 'real'); % standard parameters
...
fprintf('M_mtrx_sym is %d x %d\n', size(M_mtrx_sym,1), size(M_mtrx_sym,2));
fprintf('C_mtrx_sym is %d x %d\n', size(C_mtrx_sym,1), size(C_mtrx_sym,2));
fprintf('G_vctr_sym is %d x %d\n', size(G_vctr_sym,1), size(G_vctr_sym,2));
