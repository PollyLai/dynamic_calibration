function sol = estimate_dynamic_params_frictionless(path_to_data, idx, drvGains, baseQR, method)
% -----------------------------------------------------------------------
...
if strcmp(method, 'OLS')
    % Usual least squares
    [sol.pi_b] = ordinaryLeastSquareEstimation(Tau, Wb);
elseif strcmp(method, 'PC-OLS')
    % Physically consistent OLS using SDP optimization
    [sol.pi_b, sol.pi_s] = physicallyConsistentEstimation(Tau, Wb, baseQR);
else
...
sol.rel_std = 100*sol.std./abs([sol.pi_b]);
end
...
function [pib_OLS] = ordinaryLeastSquareEstimation(Tau, Wb)
...
function [pib_SDP, pi_full] = physicallyConsistentEstimation(Tau, Wb, baseQR)
